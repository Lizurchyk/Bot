...
TOKEN = "7164373060:AAGRUeraUmrepQptlBqCnsDVHoANHHJtCBY"  # СРОЧНО ЗАМЕНИ НА НОВЫЙ ТОКЕН!

ADMIN_ID = 1439379837  # Твой ID
CHANNEL_ID = -1003606116956  # ID канала для постов

GAMES = {
    "game1": {
        "name": "Название игры 1",
        "download_link": "https://example.com/game1.apk",
        "media": "https://play-lh.googleusercontent.com/2ROfB_jZ4DpGaYsiDjBFiHp0ARyVc45V0VycmDLnSbadiBudbyLZobL4LGKwBW-6Hg=w526-h296-rw",  # Сюда сохранится file_id фото
        "media_type": "photo",
        "post_link": "https://play-lh.googleusercontent.com/2ROfB_jZ4DpGaYsiDjBFiHp0ARyVc45V0VycmDLnSbadiBudbyLZobL4LGKwBW-6Hg=w526-h296-rw"
    },
    "game2": {
...
# ============================================
def SendGameToUser(chat_id, game_key):
...
def PublishPost(chat_id, game_key, text_message, is_test=False):
    """Публикует пост в канал, добавляя фото из games к тексту"""
    game = GAMES.get(game_key)
    if not game:
    ...
    keyboard = InlineKeyboardMarkup()
    keyboard.add(InlineKeyboardButton("📥 Скачать", url=deep_link))

    try:
        # Получаем текст поста (то что написал админ)
        post_text = text_message.text or text_message.caption or ""
        
        # Если есть фото в games - отправляем фото + текст
        if game.get('media') and game.get('media_type') == 'photo':
            sent = bot.send_photo(
                target,
                game['media'],  # file_id фото из games
                caption=post_text,
                caption_entities=text_message.entities or text_message.caption_entities,
                reply_markup=keyboard
            )
        # Если есть видео в games
        elif game.get('media') and game.get('media_type') == 'video':
            sent = bot.send_video(
                target,
                game['media'],  # file_id видео из games
                caption=post_text,
                caption_entities=text_message.entities or text_message.caption_entities,
                reply_markup=keyboard
            )
        # Если нет медиа в games, но админ прислал фото в посте
        ...
        # Если нет медиа в games, но админ прислал видео в посте
        ...
        # Если совсем нет медиа - просто текст
...
def check_admin_access(message):
    """Проверяет, что пользователь админ и подписан на каналы"""
    ...
    is_subscribed, unsubscribed = CheckSub(user_id)
    if not is_subscribed:
        keyboard = SubKeyboard(unsubscribed)
...
def text_command(message):
    ...
    is_subscribed, _ = CheckSub(ADMIN_ID)
    if not is_subscribed:
...
def handle_post_text(message):
    ...
    success, result = PublishPost(ADMIN_ID, game_key, message, is_test)
